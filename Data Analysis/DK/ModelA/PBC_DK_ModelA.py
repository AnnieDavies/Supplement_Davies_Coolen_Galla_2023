'''
Code to perform data analysis on the PBC data set in Davies, Coolen and Galla (2023) for delayed kernel model A.
...
def func(V, T, Z, Z_fix, X, N_train, p, n, Censor):
    func_val = 0.0
    if V[3] < 0 or V[4] < 0 or V[5] < 0: #penalise for negative tau (NB: for PBC data V[3], V[4] and V[5] represent tau)
        func_val = func_val + 10e15
    else:
        #sum over i (individuals in the training data)
...
def EXP_A(ti, kappa, tau, gamma, Z_t, T_t, Zf_t, n_t, j):
    ...
    for mu in range(0,p):       		
        if int(n_t[j])==1: 
            ...
            for a in range(0,int(n_t[j])-1): #sum over n[j]-1 longitudinal observations
                if tau[mu] == 0.0: 
                    summ = summ
...
###################
# Run main_func in parallel for each group
###################
